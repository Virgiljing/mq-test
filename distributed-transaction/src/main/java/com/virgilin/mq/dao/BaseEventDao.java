...
public class BaseEventDao extends JdbcDaoSupport {
    ...
    public Integer insert(Event event){
        ...
        return getJdbcTemplate().update("insert into t_event(type,process,content,create_time,update_time) values (?,?,?,now(),now())",(PreparedStatement ps)->{
    ...
    public Integer updateProcess(Event event){
        return getJdbcTemplate().update("update t_event set process=?,update_time=now() where id=?",(PreparedStatement ps)->{
